...
export default function EmailAuthModal({ open, onClose }: EmailAuthModalProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // NextAuth v5 beta: signIn with redirect:false does NOT refresh
      // the client-side session reliably. A page reload guarantees the
      // session token cookie is picked up and the UI shows the correct
      // authenticated state.
      onClose();
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <button
                onClick={onClose}
                className="w-7 h-7 rounded-full border border-white/10 flex items-center justify-center text-white/50 hover:text-white/90 text-sm"
              >
                ×
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div>
                <p
                  className="text-[10px] text-[rgba(240,237,232,0.4)] mb-1"
